...
from .formats import get_supported_formats, FormatValidationResult
from .types import ProcessingResult


def validate_request(
    file_path: Optional[str], prompt: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    return {"file_path": str(path), "prompt": prompt}


def validate_file_format(
    file_path: Union[str, Path], type_hint: Optional[str] = None
    ...
    """
    if not file_path:
        return FormatValidationResult(is_valid=False, error="Invalid filename")
    ...
    if suffix not in formats:
        return FormatValidationResult(
            is_valid=False, error=f"Invalid file format: {suffix}"
        )

    # For test files, don't validate existence
    if str(file_path).startswith("tests/"):
        return FormatValidationResult(is_valid=True, format_name=formats[suffix])

    # For regular files, check existence
    if not path.exists():
        return FormatValidationResult(
            is_valid=False, error=f"File not found: {file_path}"
        )
    ...
    if suffix == ".pdf":
        try:
            with open(path, "rb") as f:
                ...
                if header != b"%PDF":
                    return FormatValidationResult(
                        is_valid=False, error="Invalid PDF file"
                    )
        except Exception:
            return FormatValidationResult(is_valid=False, error="Error reading file")
...
def validate_response(
    ...
    if success:
        if text is None and llm_response is None:
            raise ValueError(
                "Either text or llm_response must be provided for successful results"
            )
        ...
        return ProcessingResult(
            success=False, text=None, page_count=None, llm_response=None, error=error
        )
