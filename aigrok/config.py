...
class ConfigManager:
    ...
    def _load_config(self):
        ...
        try:
            ...
            if config_dict.get("ocr") and isinstance(config_dict["ocr"], dict):
                ...
                config_dict["ocr_languages"] = config_dict["ocr"].get(
                    "languages", ["en"]
                )
    ...
    def _configure_model(
        self, model_type: str, current_config: Optional[ModelConfig] = None
    ) -> Optional[Dict]:
        ...
        while provider_idx is None:
            prompt = (
                f"\nSelect provider (number) [{default_idx}]: "
                if default_idx
        ...
        if provider_info.get("endpoint_required"):
            ...
            endpoint = (
                input(f"\nEnter {provider} endpoint [{default_endpoint}]: ").strip()
                or default_endpoint
            )
        ...
        while model_idx is None:
            prompt = (
                f"\nSelect model (number) [{default_idx}]: "
                if default_idx
                else "\nSelect model (number): "
            )
            choice = input(prompt).strip()
    ...
    def configure(self):
        ...
        print(
            "This model will be used for processing images and PDFs with visual content.\n"
        )
        ...
        enable_ocr = (
            input(
                f"\nEnable OCR support? (y/N) [{current_ocr_enabled and 'y' or 'N'}]: "
            )
            .strip()
            .lower()
        )
        if enable_ocr == "y" or (not enable_ocr and current_ocr_enabled):
            ...
            languages = input(
                f"\nEnter OCR languages (comma-separated) [{default_languages}]: "
            ).strip()
            if languages:
            ...
            fallback = (
                input(
                    f"\nEnable OCR fallback? (y/N) [{current_fallback and 'y' or 'N'}]: "
                )
                .strip()
                .lower()
            )
            config_dict["ocr_fallback"] = fallback == "y" or (
                not fallback and current_fallback
            )

            current_threshold = (
                self.config.ocr_confidence_threshold if self.config else 0.5
            )
            threshold = input(
                f"\nEnter OCR confidence threshold (0.0-1.0) [{current_threshold}]: "
            ).strip()
            if threshold:
        ...
        try:
            ...
            print(
                f"- Text: {self.config.text_model.provider} / {self.config.text_model.model_name}"
            )
            print(
                f"- Vision: {self.config.vision_model.provider} / {self.config.vision_model.model_name}"
            )
            if self.config.audio_model:
                print(
                    f"- Audio: {self.config.audio_model.provider} / {self.config.audio_model.model_name}"
                )
            if self.config.ocr_enabled:
                print(
                    f"- OCR: enabled={self.config.ocr_enabled}, languages={self.config.ocr_languages}, fallback={self.config.ocr_fallback}, confidence_threshold={self.config.ocr_confidence_threshold}"
                )
        except Exception as e:
    ...
    def _get_ollama_models(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            client = ollama.Client(
                host=self.SUPPORTED_PROVIDERS["ollama"]["default_endpoint"]
            )
            models = client.list()
