...
from aigrok import process_document, process_documents, Config
...
def custom_output_formatting():
    ...
    result = process_document(
        "data.pdf",
        format="csv",
        columns=["title", "date", "author", "content"]
    )
    ...
    result = process_document(
        "report.pdf",
        format="markdown",
        headers=["Summary", "Key Points", "Conclusion"]
    )
...
async def main():
    ...
    results = await batch_processing(files, "Summarize the content")
