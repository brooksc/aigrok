...
def test_process_single_file(mock_pdf_processor, sample_pdf):
    ...
    mock_pdf_processor.process_file.assert_called_once_with(sample_pdf, "Analyze this")
...
def test_main_single_file(mock_pdf_processor, sample_pdf, monkeypatch):
    ...
    with (
        patch("sys.argv", ["aigrok", "analyze this", str(pdf_path)]),
...
def mock_ollama():
    ...
    with patch("ollama.list") as mock:
        mock.return_value = {"models": [{"name": "llama3.2:3b"}, {"name": "llama3.2-vision:11b"}]}
        yield mock
...
def test_configure_creates_default_config(tmp_path, mock_ollama):
    ...
    with (
        patch("aigrok.config.ConfigManager.CONFIG_DIR", config_dir),
...
class TestCLI:
    ...
    def test_llm_response_formats(self, mock_args):
        ...
        dict_response = {"choices": [{"message": {"content": "test content"}}]}
