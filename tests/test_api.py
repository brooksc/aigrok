...
def test_output_schema_validation():
    ...
    # Valid JSON schema
    schema = OutputSchema(format="json", schema_def='{"title": "str", "amount": "float"}')
    assert schema.format == "json"

    # Valid CSV schema
    schema = OutputSchema(format="csv", schema_def=["title", "amount"])
...
def test_process_response_validation():
    ...
    # Success response
    response = ProcessResponse(success=True, text="Sample text", page_count=1)
    assert response.success
    assert response.text == "Sample text"
    assert response.error is None

    # Error response
    response = ProcessResponse(success=False, error="Failed to process")
...
def test_api_processor_json_output(mock_api_processor):
    ...
    schema = OutputSchema(format="json", schema_def='{"title": "string", "amount": "number"}')
...
def test_api_processor_no_ocr(mock_api_processor):
    ...
    request = ProcessRequest(file_path="test.pdf")
    mock_api_processor.pdf_processor.process_file.return_value = ProcessingResult(
        success=True, text="PDF text only", metadata={"sources": ["pdf"]}, page_count=1
    )
...
def test_api_processor_markdown_output(mock_api_processor):
    ...
    schema = OutputSchema(format="markdown", schema_def=["# Title", "## Section", "Content"])
    request = ProcessRequest(file_path="test.pdf", output_schema=schema)
