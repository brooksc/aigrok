...
def validate_test_case(test_case: Dict[str, Any]) -> None:
    ...
    for field in required_fields:
        if field not in test_case:
            raise TestCaseValidationError(f"Missing required field: {field}", test_case)
        ...
        if "max_slowdown_factor" in test_case:
            ...
            if not isinstance(factor, (int, float)) or factor <= 0:
                raise TestCaseValidationError(
                    "max_slowdown_factor must be a positive number", test_case
                )

    # Validate file exists
    file_path = Path(test_case["file"])
    if not file_path.exists():
        raise TestCaseValidationError(f"Test file not found: {file_path}", test_case)
