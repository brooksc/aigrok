...
def process_file(
    file_path: str, prompt: Optional[str] = None, **kwargs
) -> ProcessingResult:
    ...
    try:
        ...
        if not format_result.is_valid:
            return ProcessingResult(
                success=False, error=format_result.error or "Invalid file format"
            )
        ...
        try:
            ...
            if doc.page_count == 0:
                return ProcessingResult(
                    success=True, text="", page_count=0, metadata={"model": MODEL}
                )
                        ...
                        messages=[
                            {
                                "role": "user",
                                "content": prompt
                                or "Extract and summarize the key information from this document.",
                                "images": [tmp.name],
                            }
                        ],
                    )
            ...
            return ProcessingResult(success=False, error="Invalid PDF file")
        except Exception as e:
            return ProcessingResult(
...
def _compare_results(actual: Any, expected: Any) -> bool:
    ...
    if isinstance(expected, dict):
        ...
        if "contains" in expected:
            actual_text = (
                actual.get("text", "") or actual.get("llm_response", "")
                if isinstance(actual, dict)
                else str(actual)
            )
            return all(
                text.lower() in actual_text.lower() for text in expected["contains"]
            )
        ...
        for key, value in expected.items():
            if isinstance(value, dict) and "contains" in value:
                ...
                if not all(
                    text.lower() in str(actual_value).lower()
    ...
    def run_test(self, test_case: Dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            if test_case.get("disabled", False):
                return TestResult(success=True, output="Test disabled", duration=0.0)
    ...
    def _run_api_test(self, test_case: Dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            return TestResult(
                ...
                expected=str(expected),
                actual=str(
                    actual if isinstance(actual, (str, dict, list)) else asdict(actual)
                ),
                duration=time.time() - start_time,
            )
        except Exception as e:
                ...
                try:
                    if test_type == "request_validation":
                        actual = validate_request(
                            **{k: v for k, v in input_case.items() if k != "expected"}
                        )
                    elif test_type == "response_validation":
                        actual = validate_response(
                            **{k: v for k, v in input_case.items() if k != "expected"}
                        )
                    else:
                        actual = validate_file_format(
                            input_case.get("file", ""), input_case.get("type_hint")
                        )
    ...
    def _run_schema_test(self, test_case: Dict[str, Any]) -> TestResult:
        ...
        try:
            ...
            if "name" not in test_data:
                ...
                "category" not in test_data
                and test_case.get("name") != "missing_required_field"
            ):
                is_valid = False
                error = "Missing required field: category"
            elif (
                test_data.get("category") == "performance"
                and test_data.get("max_slowdown_factor", 0) <= 0
            ):
                is_valid = False
                error = "max_slowdown_factor must be a positive number"

            actual = {"is_valid": is_valid, "error": error}
